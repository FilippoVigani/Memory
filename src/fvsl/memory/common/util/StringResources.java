...
public enum StringResources {
	...
	createdIS("Created output stream serverside"), createdOS("Created input stream serverside"), reqAsk("Request ask "), createdLo("Lobby creata con id "), destroyedLo(
			"Owner of lobby left, removing lobby."), lobbies("Lobbies"), reqFf("Request fulfilled..."), reqRec("Request received..."), nullRun("runnable null"), socketErr("Socket error, closing"), gamePerf(
			"game performing: "), contentTaken("content taken"), setUpGame("Setting up game..."), startGame("Starting game"),
	...
	// Server
	sConsole("Server console"), multyTS("Multithreaded Server"), sStop("Server Stopped."), errorAcptCC("Error accepting client connection"), errorCloseS("Error closing server"), cantOpnePort(
			"Cannot open port "), upServ("Updater Server"), nullCSock("Client socket is null"), cUpAdd("Client updater added."), newC("New client on port "), serverStartedOnPort(
	...
	// CreateLobbyPage
	unJoinLo("Unable to join lobby: "), newLo("New lobby"), loName("Lobby Name"), psw("Password"), playerNum("Number of players"), coupleNum("Number of pairs"), timer("Timer"), createLo(
			"Create lobby"), back("Back"),
	// gamePage
